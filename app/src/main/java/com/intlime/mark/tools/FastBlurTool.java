...
/**
 * Created by wtu on 15-6-30.
 */
public class FastBlurTool {

    public static Bitmap blur(Bitmap bkg, View view) {
        return blur(bkg, view, 8, 4f);
    }
    ...
    public static Bitmap blur(Bitmap bkg, View view, float scaleFactor, float radius) {
        ...
        try {
            overlay = FastBlurTool.doBlur(overlay, (int) radius, true);
        } catch (Exception e) {
            e.printStackTrace();
        }

        LogTool.d("blur", System.currentTimeMillis() - startMs + "ms");
        return overlay;
    }
    ...
    private static Bitmap doBlur(Bitmap sentBitmap, int radius, boolean canReuseInBitmap) throws Exception{
        ...
        // colors on the topmost layer of the stack are either handled on
